function fish_prompt --description 'Write out the prompt'
    ...
    if [ (_git_branch_name) ]
        set -l git_branch (set_color -o yellow)(_git_branch_name)
        if [ (_is_git_dirty) ]
 ...
 (git status --porcelain | cut -c 1-2 | uniq)
    ...
    printf '%s%s%s%s%s%s%s%s%s%s%s%s%s' (set_color -o red) '[' (set_color green) $USER (set_color red) '|' (set_color blue) (echo $PWD | sed -e "s|^$HOME|~|") (set_color white) $git_info (set_color red) ']' (set_color white)
